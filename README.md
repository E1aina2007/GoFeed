...
当前主线：视频内容模块。后端已完成视频/封面上传、发布、公开列表与详情、我的视频、作者删除，并接入会话鉴权；存储侧已落地物理文件名 4 步清洗、实际存储名与用户指定名分离、DB 只存相对路径（迁移 `000002_video_file_names`）。CI 已为后端测试引入 MySQL 8.0 service；前端仍是脚手架，尚无业务页面。

下一步优先级：

1. 补齐视频模块验收测试：仓储集成测试（真实 MySQL，CI service 已就绪）+ httptest 端到端 + 用户/会话自动化测试。
2. 用户主页视频数统计（`GET /api/user/:id/profile` 的 `video_count` 落地）。
3. 基础 Feed 与前端最小闭环（登录/注册、视频流、上传与发布、我的视频）。
