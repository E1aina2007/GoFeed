...
当前主线：发布体验与运维。后端已完成草稿聚合上传、发布、公开列表与详情、我的视频、作者删除、头像上传，并接入会话鉴权；用户主页会统计已发布且未软删除的视频数量。存储侧将清洗后的物理名与用户指定名分离，并为每次保存附加不可复用对象键；DB 只存相对路径。未发布草稿到期后由 sweeper 用 token 租约认领，逐媒体持久化删除进度，最后硬删除；任何失败都不会把 `purging` 草稿恢复为可写状态。账号和已发布视频删除仍采用软删除 + 7 天宽限期。
...
| 1 | 发布体验与运维 | 增加上传进度/取消、媒体预览和更细的错误分类；持续补充日志、健康检查和 CI 回归 | 大文件上传、取消、失败重试可验证；后端、前端、浏览器和 Compose 门禁均通过 |

### 当前模块：发布体验与运维

互动模块已经完成并按后端、前端拆分提交。下一阶段继续遵循“设计契约 → 实现 → 自动化验证 → 页面验收 → 独立提交”：先稳定上传进度、取消、媒体预览和错误分类的后端契约，再接入前端交互，最后补充日志、健康检查和 CI 门禁。每个子模块完成并提交后暂停 review，不把发布体验与运维改动混在同一提交中。
