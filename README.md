...
| 已完成 | Feed 短暂故障自愈 | 对网络错误和临时 `408`/`429`/`5xx` 做最多两次退避重试；续页始终沿用原游标，销毁页面会取消等待中的重试 | `4a54301` 已完成 lint、13 个 Vitest 文件共 63 个用例、类型检查和生产构建；按当前任务要求未运行浏览器测试 |
...
| 1 | Feed 服务端可靠性 | 保持 MySQL 为唯一事实源，先评审稳定游标、批量数据补全、暂态失败语义与可观测性；不接入 Redis/RabbitMQ 客户端 | 后端方案确认后再实现；降级不得破坏已发布过滤、游标顺序或 MySQL 事实 |
| 2 | RabbitMQ 视频处理闭环 | 在 Feed 核心稳定后，以 outbox 可靠发布视频处理事件，由 worker 消费、重试并更新 `processing`/`rejected` 状态 | 重复消费、进程崩溃、死信和状态不可逆性均有测试；worker 可访问共享媒体存储 |
| 3 | Redis 定向能力 | 先接入限流、会话缓存、短期幂等键或任务进度，再依据指标评估更广缓存 | 明确 TTL、失效、降级策略和命中率；MySQL 仍是用户、视频、草稿及清扫状态的权威来源 |

### 当前阶段：Feed 可靠性优先

互动模块已经完成并按后端、前端拆分提交。草稿恢复后端已在 `7afa144` 完成并同步更新 [`API.md`](./API.md)：`GET /api/video/auth/drafts/:id` 返回当前草稿和媒体完成标识，`DELETE /api/video/auth/drafts/:id` 将草稿不可逆地排入清扫。草稿恢复前端已在 `2028f95` 完成，只消费这两个既有接口来确认上传响应不确定后的服务端事实，并提供明确的放弃草稿操作；上传取消已在 `655e878` 完成，通过可中止媒体请求和服务端草稿状态确认避免取消后误发后续请求或重新激活 `purging` 草稿；媒体预览与错误分类已在 `ccbd659` 完成，覆盖本地预览生命周期、选择前校验和可恢复错误提示；运维可观测性已在 `1b702cd` 完成，提供请求关联、健康探针和清扫轮次日志。Feed 前端短暂故障自愈已在 `4a54301` 完成，优先恢复网络和临时服务错误，且不改变游标、去重和取消约束。中间件接入准备已在 `487d700` 完成，固定 Redis/RabbitMQ 的配置、可用性和本地/CI 契约，但不改变 MySQL 业务事实源，也不把它们设为现有进程的启动必需依赖。下一步先进行 Feed 服务端可靠性方案评审；后端确认前不修改接口、迁移或 worker。

### Redis 与 RabbitMQ 引入边界

当前先不引入 Redis/RabbitMQ 客户端，也不让它们替代 MySQL 的业务事实源。`487d700` 已扩展 `config.Config` 与环境变量覆盖测试，补齐 Compose 服务、健康检查、持久卷和 CI service；Redis/RabbitMQ 使用独立健康检查，端口仅回环暴露，API 不等待或探测它们。本地开发仍以本机 MySQL 加直接运行 Go/Vue 为默认方式。后续任何客户端接入都必须先固定对应功能的故障策略和就绪语义，而不是由连接失败时的临时重试决定。

Feed 服务端可靠性完成后，RabbitMQ 的首个业务闭环才考虑视频异步处理，而不是把现有清扫租约搬到队列：

1. 发布请求确认原始媒体完整后，在同一数据库事务中将视频置为 `processing` 并写入 outbox 事件。
2. relay 发布持久化消息；消息只携带视频 ID、对象路径、事件 ID 和版本，不携带文件内容。
3. worker 手动 ack，按事件 ID 或消费记录保证幂等；失败按有限次数重试，超过次数进入死信队列。
4. worker 只允许 `processing -> published` 或 `processing -> rejected`，并提供状态查询；API、worker 和 sweeper 必须访问同一份媒体存储。

Redis 先承担短生命周期的保护和加速职责：登录/注册/上传限流、会话校验缓存、短期幂等键以及异步进度。会话缓存 TTL 不得超过会话有效期，注销、改密和账号删除必须主动失效；Redis 故障时要明确降级到 MySQL 还是拒绝请求。草稿的 `purging` 状态、token 租约和媒体检查点继续由 MySQL 管理，当前游标 Feed 也不做 Redis 缓存，除非指标证明确有读压力。
...
后端回归从 `backend` 执行 `go test ./...`；前端回归使用 `pnpm run lint`、`pnpm run test:unit -- --run`、`pnpm run build`。浏览器测试按需执行，本次草稿恢复前端模块不作为提交门槛；真实发布、鉴权和媒体流仍按上述本地 MySQL 验收清单联调。部署配置改动还必须执行 `docker compose config --quiet`。
