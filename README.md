...
| 顺序 | 模块 | 主要工作 | 完成标准 |
| --- | --- | --- | --- |
| 1 | Feed 稳定性 | 为首屏和游标分页增加请求取消、并发锁、视频 ID 去重；路由切换和页面失焦时暂停播放；保留加载、空状态、错误重试和到底提示 | 快速滚动不会重复请求或重复渲染；旧请求返回不会覆盖新数据；桌面和移动端播放状态正确；补齐 Feed 单测和移动端验收 |
| 2 | 基础 Feed 回归 | 按验收流程回归认证、发布、Feed、详情、作者、我的视频、账户设置和提示反馈；修复跨页面状态同步问题 | 本地 MySQL + 直接启动前后端可完整走通；API 状态码、错误提示和跳转符合 `API.md`；桌面/移动端截图无布局重叠 |
| 3 | 互动后端基础 | 先设计点赞、评论、关注的数据模型和迁移，再实现 repository/service/controller/路由及鉴权；同步更新 `API.md` | 每类互动都有成功、重复操作、未登录、资源不存在和软删除场景测试；迁移可独立执行和回滚 |
| 4 | 互动前端页面 | 在后端契约稳定后接入点赞、评论、关注按钮和列表；复用现有 API 错误处理与 toast | 未登录操作能跳转登录；重复点击有明确状态；网络失败可重试；Feed、详情、作者主页状态一致 |
| 5 | 发布体验与运维 | 增加上传进度/取消、媒体预览和更细的错误分类；补充日志、健康检查和 CI 回归说明 | 大文件上传、取消、失败重试可验证；`go test ./...`、前端 type-check/lint/test/build 和 Compose 配置检查均通过 |

### 模块 1 的实施拆分

模块 1 仍属于前端单模块，建议按以下小步完成，但最终只在该模块整体验收后提交：

1. 在 `frontend/src/features/video` 统一请求生命周期，使用 `AbortController` 取消失效的首屏/分页请求。
2. 在 `FeedView.vue` 增加请求序列号或游标锁，保证快速滚动时只允许一个分页请求，响应按视频 ID 合并去重。
3. 监听 `visibilitychange` 和路由离开事件，页面不可见或离开 Feed 时暂停所有播放器，返回后只恢复当前可见视频。
4. 为取消请求、乱序响应、重复视频和播放暂停补充单测，并用桌面与移动端 Playwright 验证滚动、重试和播放行为。

模块 1 完成后暂停 review；review 通过后再进入模块 2，不提前混入点赞、评论或关注代码。
...
后端回归从 `backend` 执行 `go test ./...`；前端回归使用 `pnpm.cmd run type-check`、`pnpm.cmd run test:unit -- --run`、`pnpm.cmd exec eslint .`、`pnpm.cmd exec oxlint .` 和 `pnpm.cmd run build`。
