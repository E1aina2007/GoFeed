...
当前下一步是 **Feed 服务端可靠性**：先确认 MySQL 事实源、发布数据不变量、keyset 游标、作者批量补全和暂态失败语义；在方案评审与后端回归完成前，不接入 Redis/RabbitMQ 客户端，也不先修改前端交互。后续路线为 RabbitMQ 视频处理闭环，再按指标引入 Redis 定向能力。
