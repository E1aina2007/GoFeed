...
- 视频冗余计数字段（`likes_count`、`comments_count`）的移除由阶段二 R1 迁移处理；互动统计故障语义已完成，下一步进入可观测性与查询预算（慢查询阈值、请求内查询计数与路由级预算断言）
...
- 下一模块：可观测性与查询预算；配置慢查询阈值、注册请求内查询计数，并为公开列表与详情建立路由级预算断言
