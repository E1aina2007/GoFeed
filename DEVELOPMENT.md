...
| Feed 数据不变量与查询边界 | 已实现（待 review） | 工作树（未提交） | 公开详情、Feed、我的视频、作者视频计数和互动视频校验统一排除残缺记录 |
...
- 当前公开视频条件使用 GORM `clause` 参数化值，但查询入口仍要求调用方先绑定 `Video` 模型；下一模块将把模型绑定与公开边界组合为单一入口，不增加额外 SQL 查询
- 公开视频列表使用 `(published_at DESC, id DESC)` keyset 游标，并通过 `limit+1` 判断是否有下一页
- 当前游标只编码发布时间和视频 ID，未包含版本、查询范围绑定或签名
...
1. **Feed 数据不变量与查询边界（已实现，待 review）**：公开详情、列表、我的视频、作者计数和互动资源校验统一要求发布状态、未软删除、非空发布时间及完整媒体字段；服务层对异常实体 fail-closed，真实 MySQL 与路由回归已覆盖残缺记录
2. **模型绑定的公开视频查询入口（方案已确定）**：将 scope 设为 `video` 包内私有，导出 `PublicVideoQuery(db)` 固定 `Model(&Video{})` 与公开视频条件；视频和互动仓储只追加作者、游标等局部条件，不新增迁移、API、前端或额外查询
3. **游标契约**：保留 `(published_at, id)` 的 keyset 顺序和 `limit+1`；决定游标是否加入版本、查询范围绑定（例如 `author_id`）或签名，并为过期、篡改、跨范围复用和同时间戳数据定义错误语义
...
当前 `PublicVideoScope` 已使用 GORM 的结构化条件和值参数绑定，主要剩余风险不是 SQL 注入，而是调用方遗漏 `Model(&Video{})` 或手写列名与模型漂移。下一模块先收紧入口：
...
- Feed 作者 N+1、互动统计失败语义和视频冗余计数字段仍是阶段一的决策点；数据不变量模块已完成，下一步先收紧公开视频查询入口，再进入游标契约
...
- 本模块未修改迁移、`API.md`、前端或游标格式；工作树改动暂不提交，等待 review
- 下一模块：模型绑定的公开视频查询入口；先固定 `PublicVideoQuery` 的模型绑定、软删除和 scope 复用，再进入游标契约并决定游标版本、查询范围绑定和篡改/过期错误语义
