...
- 视频冗余计数字段（`likes_count`、`comments_count`）的移除由阶段二 R1 迁移处理；可观测性与查询预算已完成，阶段一仅剩并发与异常测试收尾（M6）
...
- 下一模块：并发与异常测试收尾（阶段一收口）；覆盖同刻发布排序、分页期间新增/软删除、作者注销、游标篡改、统计失败 503、数据库暂态失败与 GET 幂等矩阵
