...
1. **Feed 数据不变量与查询边界**：明确公开项必须满足 `status = 'published'`、未软删除、`published_at IS NOT NULL`、视频和封面媒体字段完整且符合存储契约；让查询、实体映射和测试共同守住该不变量
2. **游标契约**：保留 `(published_at, id)` 的 keyset 顺序和 `limit+1`；决定游标是否加入版本、查询范围绑定（例如 `author_id`）或签名，并为过期、篡改、跨范围复用和同时间戳数据定义错误语义
3. **作者批量补全**：在不改变 video → user 依赖方向的前提下增加批量公开作者读取，消除列表中逐作者查询；缺失或已注销作者继续返回稳定占位资料
4. **互动统计故障语义**：保持按视频 ID 批量查询，明确统计查询失败时返回 `503`、整体失败还是可接受的降级值，禁止无记录地把错误伪装成零计数
5. **可观测性与查询预算**：记录请求/查询失败及耗时，增加列表查询次数、作者批量查询和统计批量查询的断言，避免性能回归被功能测试掩盖
6. **并发和异常测试**：覆盖同一 `published_at` 的 ID 排序、分页期间新增/软删除、作者注销、缺失媒体、重复游标、数据库暂态失败、重复请求和响应组装失败
...
- Feed 作者 N+1、互动统计失败语义和视频冗余计数字段仍是阶段一的决策点
