...
7. **并发和异常测试（已完成，`d185645`）**：以表级故障注入夹具在真实 MySQL 上覆盖同刻发布排序、分页期间新增/软删除、作者注销占位、统计失败 503、暂态错误干净路径与 GET 幂等；残缺记录排除与游标 400 矩阵引用既有回归
...
- 先执行未应用迁移，再按以下顺序做基础验收：
  ...
  3. 上传视频和封面并发布，Feed 能展示封面、标题和作者入口（R1 后发布先进入 `processing`；R2 已落地，本地启动 rabbitmq 服务并运行 worker 后自动流转为可见）
