...
> 更新日期：2026-09-12
>
> 后端功能基线：`7f97382`（视频处理消息可靠性已提交）；真实 RabbitMQ 重连回归补充为 `ef94396`，文档提交不单独推进功能状态
...
| 二 | R4 MQ 可靠性增强 | MQ 规格、运行中重连、outbox 租约、分级重试/DLQ | 无 | 无 | 已提交 `7f97382`、`ef94396`；核心真实依赖回归通过，待运维收口 |
...
`7f97382` 已完成固定 MQ 规格、运行中 connection 重连、outbox claim 租约/退避与 consumer ACK/分级重试/DLQ，`ef94396` 增加真实 broker 断线重连和拓扑恢复回归。`Runtime.Close` 是终止操作，不被测试或业务代码用作“模拟断线”；意外断线才触发下一次调用重建连接和拓扑。MySQL outbox 仍是事件派发状态的事实源，RabbitMQ 只承载至少一次投递。核心真实依赖路径已经通过；剩余工作是目标库升级、5s/30s 延迟计时、整进程故障窗口，以及积压/最老事件、发布失败、接管、重连与 DLQ 观测。
